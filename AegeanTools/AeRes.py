...
def load_sources(filename,
    ...
    for c in required_cols:
        if c not in table.colnames:
            logger.error(f"Column {c} not found")
            good = False
    if not good:
    ...
    logger.info(f"read {len(catalog)} sources from {filename}")
    return catalog
...
def make_model(sources, shape, wcshelper, mask=False, frac=None, sigma=4):
    ...
    for src in sources:
        ...
        if not 0 < xo < shape[0]:
            logger.debug(f"source {src.island} is not within image")
            continue
        if not 0 < yo < shape[1]:
            logger.debug(f"source {src.island} is not within image")
            continue
        ...
        if logger.isEnabledFor(logging.DEBUG):  # pragma: no cover
            logger.debug(f"Source ({src.island},{src.source})")
            logger.debug(f" xo, yo: {xo} {yo}")
            logger.debug(f" sx, sy: {sx} {sy}")
            logger.debug(f" theta, phi: {theta} {phi}")
            logger.debug(f" xoff, yoff: {xoff} {yoff}")
            logger.debug(" xmin, xmax, ymin, ymax: {}:{} {}:{}".format(
                            xmin, xmax, ymin, ymax))
            logger.debug(" flux, sx, sy: {} {} {}".format(
                          src.peak_flux, sx, sy))
        ...
        if mask:
            ...
            m[x, y] += model
        i_count += 1
    logger.info(f"modeled {i_count} sources")
    return m
...
def make_residual(fitsfile, catalog, rfile, mfile=None,
    ...
    logger.info(f"wrote residual to {rfile}")
    if mfile is not None:
        hdulist[0].data = model
        hdulist.writeto(mfile, overwrite=True)
        logger.info(f"wrote model to {mfile}")
    return
