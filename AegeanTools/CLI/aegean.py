...
def check_projection(filename, options, log=logger):
    ...
    if not ("SIN" in header["CTYPE1"]):
        if options.imgpsf is None:
            ...
            logger.warning(
                "For projection {} you should consider supplying a psf via"
...
def main():
    ...
    logger.info(f"This is Aegean {__version__}-({__date__})")
    ...
    if options.file_versions:
        logger.info(
            "AegeanTools {} from {}".format(
                AegeanTools.__version__, AegeanTools.__file__
            )
        )
        logger.info(f"Numpy {np.__version__} from {np.__file__} ")
        logger.info(f"Scipy {scipy.__version__} from {scipy.__file__}")
        logger.info(
            f"AstroPy {astropy.__version__} from {astropy.__file__}"
        )
        logger.info(f"LMFit {lmfit.__version__} from {lmfit.__file__}")
        return 0
    ...
    if options.cores is None:
        ...
        logger.info(f"Found {options.cores} cores")
    logger.info(f"Using {options.cores} cores")

    hdu_index = options.hdu_index
    if hdu_index > 0:
        logger.info(f"Using hdu index {hdu_index}")
    ...
    if options.beam is not None:
        ...
        logger.info(
            "Beam is {} deg x {} deg with pa {}".format(
    ...
    if options.priorized > 0:
        ...
        if not os.path.exists(options.input):
            logger.error(f"{options.input} not found")
            return 1
        logger.info("Priorized fitting of sources in input catalog.")

        logger.info(f"Stage = {options.priorized}")
    ...
    logger.info(f"found {len(sources)} sources total")
