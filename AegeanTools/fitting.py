...
def nthreedgaussian_lmfit(params): 
    ...
    def rfunc(v, x, y): #TODO: Update the Doc string
        ...
        for i in range(params["components"].value):
            ...
            if result is not None:
                result += elliptical_gaussian_with_alpha(x, y, v, amp, xo, yo, nu0, sx, sy, theta, alpha) # TODO: Pass the frequency into this, which is the current frequency
            else:
                result = elliptical_gaussian_with_alpha(x, y, v, amp, xo, yo, nu0, sx, sy, theta, alpha)
        return result
