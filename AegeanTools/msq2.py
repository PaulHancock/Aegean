...
class MarchingSquares():
    ...
    def _blankWithin(self, perimeter):
        ...
        # Method:
        # perimeter starts at the top-left most pixel of a region
        # so we can 'scan right' from each 'DOWN' pixel in the perimeter, blanking as we go
        # stop when we reach an already blanked pixel

        # include the first pixel as the last pixel so we have a complete circuit.
        circuit = perimeter+[perimeter[0]]
        for i, p in enumerate(circuit[:-1]):
            # figure out which direction we are going to be moving in
            drn = classify_next(p, circuit[i+1])
            if drn == self.DOWN:
                for j in range(p[0], self.xsize):
                    q = j, p[1]
                    if self.data[q] == 0:  # stop when we reach an already blank pixel
                        break
                    else:
                        self.data[q] = 0
            if drn == self.UP:
                for j in range(self.xsize-1, p[0], -1):from copy import copy
import logging
import numpy as np
...
class MarchingSquares():
    ...
    def _blankWithin(self, perimeter):
        ...
        # so we can 'scan right' from each 'DOWN' pixel in the perimeter, blanking as we go
        # stop when we reach an already blanked pixel
        # This will not work if there are 'inclusions' within the region.
        ...
        for i, p in enumerate(circuit[:-1]):
            ...
            drn = classify_next(p, circuit[i+1])
            if drn == self.DOWN:
                for j in xrange(p[0], self.xsize):
                    q = j, p[1]
                    if self.data[q] == 0:  # stop when we reach an already blank pixel
                        break
                    else:
                        self.data[q] = 0
            elif drn == self.RIGHT:  # we don't blank right from a "RIGHT" pixel, we just blank that pixel.
                if p[1] < self.ysize:  # look out for the edge of the data
                    self.data[p] = 0
        return
