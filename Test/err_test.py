...
def multi_gauss(data, rmsimg, parinfo):
    ...
    z = np.ravel(ae.ntwodgaussian([1, 1, 1, smoothing, smoothing, 0])(x,y))[mask]
    C = np.vstack((z,)*len(z))
...
ae.save_catalog('test2.vot',sources)
ae.save_catalog('test.reg',sources)
