...
def two_d_gaussian(x, y, amp, xo, yo, major, minor, pa, flags=None):
    """
    x,y -> pixels
    xo,yo -> location
...
def lm_make_model(data,rmsimg,curve,beam,innerclip,offsets=(0,0)):
    """Estimates the number of sources in an island and returns initial parameters for the fit as well as
    limits on those parameters.
    ...
    for summit,xmin,xmax,ymin,ymax in summits:
        ...
        model = lmfit.Model(two_d_gaussian, independent_vars=['x','y'])
        model.set_param_hint('amp',value=amp, min=amp_min, max=amp_max)
        model.set_param_hint('xo', value=xo, min=xo_min, max=xo_max)
        model.set_param_hint('yo', value=yo, min=yo_min, max=yo_max)
        model.set_param_hint('major', value=major, min=major_min, max=major_max, vary = (flag & flags.FIXED2PSF)>0)
        model.set_param_hint('minor', value=minor, min=minor_min, max=minor_max, vary = (flag & flags.FIXED2PSF)>0)
        model.set_param_hint('pa', value=pa, min=-math.pi, max=math.pi, vary=(flag & flags.FIXED2PSF)>0)
        model.set_param_hint('flags',value=summit_flag, vary=False) #a dummy variable that I wish to store.
        model.prefix = 'c{d}_'.format(i)
        i+=1

    if i==1:
        model=island_model[0]
    elif i>0:
        model = reduce(lambda x,y: x+y,island_model)
    else:
        model=None
    if debug_on:
        logging.debug("Estimated sources: {0}".format(i))
...
def lm_fit_island():
    ...
    model = lm_make_model(isle.pixels,rms,icurve,beam,innerclip,offsets=[xmin,ymin])
    ...
    num_summits = len(model.components)
    ...
    if is_flag & flags.NOTFIT:
        lm = model.make_params()
    else:
        #do the fitting
        result = model.fit(isle.pixels,)
